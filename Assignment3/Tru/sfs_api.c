...
int mksfs(int fresh){
  ...
  if(fresh == 1){
    ...
    if(DIRECTORY_SIZE > 12){     //check to see if we need to use indirectPointer
      inode[0].indirectPointer = findFree();
      setAlloc(inode[0].indirectPointer);
      unsigned int *buff = malloc(BLOCKSIZE);
      write_blocks(START + inode[0].indirectPointer, 1, buff);
      free(buff);
    }
    ...
    for(k = 0; k < DIRECTORY_SIZE; k++){
      ...
      if(k > 11){
        unsigned int *buff = malloc(BLOCKSIZE);
        read_blocks(START+ inode[0].indirectPointer, 1, buff);
        ...
        inode[0].directPointers[k] = DIRECTORY_LOCATION + k;
      }
...
int sfs_fopen(char *name) {
  ...
  for(i = 0; i < MAX_FILES; i++){
    ...
    if(strncmp(rootDir[i].filename, "\0", 1) == 0 && rootDir[i].inode == 5000){
      ...
      inodeTable[inode].directPointers[0] = writeLoc;
  ...
  for(i = 0; i < MAX_FILES; i++){
    if(strncmp(rootDir[i].filename, file, MAXFILENAME + 1) == 0 && rootDir[i].inode != 5000){  //if we find the file remove data and set space to free
      ...
      if(inodeRemove->linkCount > 12){ //update i-node data
        unsigned int *buff = malloc(BLOCKSIZE);
        read_blocks(START+ inodeRemove->indirectPointer, 1, buff);
        ...
        setFree(inodeRemove->indirectPointer);
        inodeRemove->indirectPointer = 5000;
      }
      for(k = 0; k < 12; k++){
        setFree(inodeRemove->directPointers[k]);
        inodeRemove->directPointers[k] = 5000;
      }
      //update i-node data
      inodeRemove->mode = 0;
...
int sfs_fwrite(int fileID, const char *buf, int length){
  ...
  if(block > 139){ //if we have reached max file size return error
    ...
    read_blocks(START+ inode->indirectPointer, 1, indirectBuff);
    writeLoc = indirectBuff[block - 12];
    free(indirectBuff);
  }
  else{             //if not get location of block in direct pointers
    writeLoc = inode->directPointers[block];
  }
  ...
  while(length > 0){ //perform write and shift data if still data to write
    ...
    if(length > 0){  //if there is data still to write update writeloc and allocate memory
      if(block > 139){
      ...
      else if(eofBlock < block){ //set up single indirect pointer, if there is not one set up set
        if(block == 12 && inode->indirectPointer == 5000){
          int indirPtr = findFree();
          setAlloc(indirPtr);
          inode->indirectPointer = indirPtr;
        }
        ...
        if(block > 11){         //update i-node associated with file
          ...
          read_blocks(START+ inode->indirectPointer, 1, nextBuff);
          nextBuff[block - 12] = writeLoc;
          write_blocks(START+ inode->indirectPointer, 1, nextBuff);
          free(nextBuff);
        }
        else{
          inode->directPointers[block] = writeLoc;
        }
...
int sfs_fread(int fileID, char *buf, int length){ //returns -1 for failure
  ...
  if(block > 139){
    ...
    readLoc = inode->directPointers[block];
  }
  while(length > 0){
